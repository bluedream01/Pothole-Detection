...
Throttle : MCP4725 DAC  →  MY1016 controller throttle-signal wire
Rider    : twist-grip on an MCP3008 ADC channel (read back, then governed)
...
THROTTLE GOVERNOR (the new part)
────────────────────────────────
The MCP4725 *is* the throttle now, so the rider's twist-grip is read on an
ADC channel and passed THROUGH the Pi to the DAC. A safety "cap" (0..1) sits
on top of the rider's request:

        out = min(rider_request, cap)          →  DAC  →  controller

  • Normal            : cap = 1.0   (rider passes straight through)
  • Pothole detected  : cap ramps down to POTHOLE_THROTTLE_CAP for a few seconds
  • Accident confirmed: cap = 0.0 instantly (full cut, no ramp)

Because out is a *minimum*, the governor is only ever a ceiling — it can slow
the rider down, but it can never create throttle on its own. The cap ramps
(slew-rate limited) so the bike is never jerked by a voltage step, except on a
confirmed crash where it cuts immediately.
...
# SPI (MCP3008 ADC — shared by pulse sensor AND rider twist-grip)
SPI_BUS    = 0
SPI_DEVICE = 0
PULSE_CH   = 0       # MCP3008 channel for the pulse sensor
THROTTLE_CH = 1      # MCP3008 channel for the rider's twist-grip

# ── Throttle DAC (MCP4725) ─────────────────────────────────────────
DAC_I2C_ADDR = 0x60        # confirm with:  i2cdetect -y 1   (0x60/0x61 generic boards)
DAC_VDD      = 3.3         # MUST match the voltage you power the MCP4725 from.
                           # 3.3 V keeps I2C native (no level shifter) but caps
                           # VOUT at 3.3 V → you cannot exceed ~70-80% throttle.

# Measure these on YOUR controller with the original hall throttle + a multimeter:
V_THROTTLE_IDLE = 1.00     # volts the controller reads as "no throttle" (rest twist)
V_THROTTLE_MAX  = 3.30     # volts = the max throttle you allow  (must be <= DAC_VDD)

# Raw MCP3008 counts (0..1023) read from the rider grip at rest / full twist:
ADC_THROTTLE_MIN = 200     # raw count at rest twist
ADC_THROTTLE_MAX = 800     # raw count at full twist

# ── Throttle governor behaviour ────────────────────────────────────
POTHOLE_THROTTLE_CAP = 0.30   # slow to 30% ceiling on a pothole
POTHOLE_SLOWDOWN_S   = 3.0    # hold the reduced cap this long after detection
CAP_RATE_DOWN        = 2.5    # cap units/second while tightening  (fast, ~0.4 s)
CAP_RATE_UP          = 0.5    # cap units/second while releasing   (gentle, ~2 s)
THROTTLE_TICK_S      = 0.02   # 50 Hz control loop

# For bench testing without a physical grip wired to the ADC:
#   "adc"   = read the real twist-grip on THROTTLE_CH
#   "fixed" = pretend the rider is holding RIDER_THROTTLE_FIXED
RIDER_THROTTLE_SOURCE = "adc"
RIDER_THROTTLE_FIXED  = 0.6
...
RESET_TIMEOUT_S = 5   # set to 0 to require manual button press
...
_INITIAL_STATE = {
    ...
    # ── throttle governor ──
    "pothole_until"      : 0.0,     # slowdown active while time.time() < this
    "throttle_cap"       : 1.0,     # current governor ceiling (ramped, 0..1)
    "throttle_out"       : 0.0,     # last fraction actually sent to the DAC
    "rider_request"      : 0.0,     # last rider grip fraction read (0..1)
}
...
# Bus locks — serialize our own access to each shared bus.
#   i2c_lock : MPU9250 reads + all MCP4725 writes (both use smbus2 on bus 1)
#   spi_lock : pulse sensor + rider grip (both use the one MCP3008)
...
def _read_mpu9250():
    # Whole 6-word burst under the lock so a DAC write can't slip between the
    # register reads and so we never share the bus mid-transaction.
    with i2c_lock:
...
# ──────────────────────────────────────────────────────────────────
#  MCP4725 DAC HELPERS  (throttle output)
# ──────────────────────────────────────────────────────────────────
# The 12-bit code we send maps linearly to VOUT:   VOUT = (code / 4095) * DAC_VDD
# We only ever drive between the idle voltage and the max-allowed voltage, so a
# fraction of 0.0 = "no throttle" (idle) and 1.0 = "max allowed throttle".

_DAC_CODE_IDLE = round(4095 * V_THROTTLE_IDLE / DAC_VDD)
_DAC_CODE_MAX  = round(4095 * V_THROTTLE_MAX  / DAC_VDD)

def _dac_write(code12: int):
    """Fast-mode write: updates the DAC register only (not EEPROM)."""
    code12 = max(0, min(4095, int(code12)))
    with i2c_lock:
        # byte1 = 0000 D11 D10 D9 D8   (top nibble command bits = 0, normal power)
        # byte2 = D7..D0
        i2c_bus.write_i2c_block_data(DAC_I2C_ADDR, (code12 >> 8) & 0x0F,
                                     [code12 & 0xFF])

def _dac_write_fraction(frac: float):
    """frac 0.0..1.0  →  idle-voltage .. max-voltage."""
    frac = max(0.0, min(1.0, frac))
    code = round(_DAC_CODE_IDLE + frac * (_DAC_CODE_MAX - _DAC_CODE_IDLE))
    _dac_write(code)

def _dac_write_eeprom_idle():
    """
    Store the idle code in the MCP4725's EEPROM so that on the NEXT power-up the
    throttle line sits at idle *before* this program has even started. Called
    once at init — do not spam it (EEPROM has limited write cycles).
    """
    code = _DAC_CODE_IDLE
    with i2c_lock:
        # cmd 0x60 = "write DAC register and EEPROM"
        # data = D11..D4 , D3..D0 0000
        i2c_bus.write_i2c_block_data(DAC_I2C_ADDR, 0x60,
                                     [(code >> 4) & 0xFF, (code << 4) & 0xFF])
    time.sleep(0.06)   # EEPROM write settle (~25-50 ms)

# ──────────────────────────────────────────────────────────────────
#  MCP3008 SPI ADC HELPER  (thread-safe — pulse + throttle share it)
# ──────────────────────────────────────────────────────────────────
...
def _read_rider_throttle() -> float:
    """Rider twist-grip position as a 0.0..1.0 fraction."""
    if RIDER_THROTTLE_SOURCE == "fixed":
        return max(0.0, min(1.0, RIDER_THROTTLE_FIXED))
    raw  = _read_adc(THROTTLE_CH)
    span = max(1, ADC_THROTTLE_MAX - ADC_THROTTLE_MIN)
    frac = (raw - ADC_THROTTLE_MIN) / span
...
def _reset_system(source: str = "manual"):
    ...
    • server_message
    • pothole_until      (governor cap ramps back up on its own)
    ...
    NOTE: this does NOT force the throttle open. It only clears the accident
    flag; the governor thread then ramps the cap back up gently, and the bike
    still only moves if the rider is actually holding the grip.
    """
    ...
    with state_lock:
        ...
        state["pothole_until"]      = 0.0           # let the cap ramp back up
...
def _trigger_accident(accident_type: str):
    ...
    buzzer.on()
    # The throttle governor thread sees accident_confirmed and cuts to 0 instantly.
...
# ──────────────────────────────────────────────────────────────────
#  THROTTLE GOVERNOR THREAD
#  Reads the rider grip, applies the safety cap, drives the MCP4725.
#  This is the ONLY place that writes throttle output during normal running.
# ──────────────────────────────────────────────────────────────────

def throttle_thread():
    cap = 1.0
    _dac_write_fraction(0.0)          # start at idle no matter what
    print("[INIT] Throttle governor running")

    while True:
        now = time.time()
        with state_lock:
            accident = state["accident_confirmed"]
            pothole  = now < state["pothole_until"]

        if accident:
            # Confirmed crash → cut immediately, skip the ramp.
            cap = 0.0
        else:
            target = POTHOLE_THROTTLE_CAP if pothole else 1.0
            rate   = CAP_RATE_DOWN if target < cap else CAP_RATE_UP
            step   = rate * THROTTLE_TICK_S
            if cap < target:
                cap = min(target, cap + step)
            elif cap > target:
                cap = max(target, cap - step)

        rider = _read_rider_throttle()
        out   = min(rider, cap)       # governor is a ceiling, never a source

        try:
            _dac_write_fraction(out)
        except Exception as e:
            print(f"[THROTTLE] DAC write failed: {e}")

        with state_lock:
            state["throttle_cap"]  = cap
            state["throttle_out"]  = out
            state["rider_request"] = rider

        time.sleep(THROTTLE_TICK_S)
            ...
            if resp.ok:
                ...
                if msg:
                    with state_lock:
                        ...
                        # Feed the pothole event straight to the governor:
                        if "Pothole has been detected" in msg:
                            state["pothole_until"] = time.time() + POTHOLE_SLOWDOWN_S
...
def main():
    ...
    # Bring the throttle DAC up to a known-safe idle BEFORE anything can move.
    try:
        _dac_write_eeprom_idle()      # power-up default = idle (persists across reboots)
        _dac_write_fraction(0.0)      # and drive idle right now
        print(f"[INIT] MCP4725 ready @ 0x{DAC_I2C_ADDR:02X} "
              f"(idle code {_DAC_CODE_IDLE}, max code {_DAC_CODE_MAX})")
    except Exception as e:
        print(f"[INIT] MCP4725 failed: {e} — throttle control disabled")
    ...
    print(f"\n[INIT] All systems up.\n"
          ...
          f"       Throttle DAC    : 0x{DAC_I2C_ADDR:02X} @ {DAC_VDD} V  "
          f"(idle {V_THROTTLE_IDLE} V → max {V_THROTTLE_MAX} V)\n"
          f"       Pothole cap     : {POTHOLE_THROTTLE_CAP:.0%} for {POTHOLE_SLOWDOWN_S}s\n"
          f"       Rider source    : {RIDER_THROTTLE_SOURCE}\n")
    ...
    try:
        while True:
            with state_lock:
                ...
                cap       = state["throttle_cap"]
                out       = state["throttle_out"]
                pothole   = time.time() < state["pothole_until"]
            ...
            if accident:
                ...
                status = "Buzzer ON" if buzzer_on else "Buzzer OFF"
            ...
            if "Pothole has been detected" in srv_msg:
                ...
                oled_show(f"POTHOLE!\nSlowing to {POTHOLE_THROTTLE_CAP:.0%}\nout {out:.0%}")
                with state_lock:
            ...
            if dist <= DIST_COLLISION:
                ...
                # Normal running: show live throttle so you can watch the governor.
                gov = "CAP" if (pothole or cap < 0.999) else ""
        ...
        led.off()
        try:
            _dac_write_fraction(0.0)   # drop throttle to idle BEFORE closing the bus
        except Exception:
            pass
